...
class TourgenDistanceMatrix implements Serializable {
	...
	/**
	 * return an array of long representing the distance from one location to a list
	 * of Locations.
	 * 
	 * @param a
	 *            center point
	...
	private static long[] getGeometricDistance1ToN(Location a, List<Location> locationList) {
		...
		for (Location i : locationList) {
			...
			if (!instance.get(origName).keySet().contains(i.getName())) {
				distance = (long) getGeometricDistance(a.getLatitude(), a.getLongitude(), i.getLatitude(),
						i.getLongitude());
	...
	private static long[] getDrivingDistance1ToN(Location a, List<Location> locationList) {
		/*
		 * if (instance == null) { instance = new HashMap<String, HashMap<String,
		 * Long>>(); } if (!instance.keySet().contains(a.getName())) { String[]
		 * origLatLon = new String[] { a.getCoordinateString() }; String[] destLatLons =
		 * new String[locationList.size()]; for (int i = 0; i < locationList.size();
		 * i++) { destLatLons[i] = locationList.get(i).getCoordinateString(); } long[][]
		 * distance = helper.getDistance(origLatLon, destLatLons); if (distance != null
		 * && distance.length == 1 && distance[0].length == locationList.size()) {
		 * String origLocationName = a.getName(); for (int i = 0; i <
		 * distance[0].length; i++) { if (instance.get(origLocationName) == null) {
		 * instance.put(origLocationName, new HashMap<String, Long>()); }
		 * instance.get(origLocationName).put(locationList.get(i).getName(),
		 * distance[0][i]); } return distance[0]; } else { return null; } } else {
		 * 
		 * java.util.Collection distancesFromStorage =
		 * instance.get(a.getName()).values(); Object[] distanceObjectArray =
		 * distancesFromStorage.toArray(); long[] distanceLongArr = new
		 * long[distanceObjectArray.length]; for (int i = 0; i <
		 * distanceObjectArray.length; i++) { distanceLongArr[i] = (long)
		 * distanceObjectArray[i]; } return distanceLongArr; }
		 */
		return null;
	}
