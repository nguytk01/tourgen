...
public class Meet implements Serializable {
	...
	public static final String SCHOOL_REMOVED = "School removed";
	public static final String SCHOOL_ADDED = "School added";
	public static final String HOST_SCHOOL_CHANGED = "Host school changed";
	double maxDistance = 0;
	double averageDistance = 0;

	/**
	 * construct a meet from the given stage and the given meetDate. The meetDate
	 * paramter is obsolete. It is there for backward compatibility. The client can
	 * pass a null object.
	 * 
	 * @param meetStage
	 *            the stage of the meet.
	 * @param meetDate
	 *            the meeting date of the meet (obsolete).
	 */
	...
	public double[] getMaxAndAvgDistance() {
		...
		if (meetStage.getStageType() != StageType.SECTIONAL) {
			...
			poolOfSchools = participantSchools;
		}
	...
	void recursiveUpdateSectionalSchoolsList() {
		if (meetStage.getStageType().equals(StageType.SECTIONAL)) {
			...
			for (School school : participantSchools) {
				try {
					sectionalSchoolsOfMeet.addAll(this.getStage().getTournament()
							.getStageOfStageType(meetStage.getStageType().getLowerStageType())
							.getMeetOfHostSchool(school).getSectionalSchoolsOfMeet());
				} catch (NullPointerException e) {
					System.out.print("Stage type " + meetStage.getStageType() + " Lower Stage type "
							+ meetStage.getStageType().getLowerStageType());
